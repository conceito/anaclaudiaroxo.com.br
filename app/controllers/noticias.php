...
//        $this->breadcrumb->add('Notícias');
        ...
        $posts = $this->post->setModuleId(7)->setPerPage(15)->setBaseUrl('noticias')->getAll();
        
        $v['posts'] = $posts;
        
        // string de navegação para injetar na view
        $v['pagination'] = $this->cms_posts->pagination();
        $v['total'] = $this->cms_posts->get_total();
//        dd($v['posts'], true);
        
        
        $this->title = 'Notícias';
        $this->corpo = $this->load->view('noticias', $v, true);
...
//        $this->breadcrumb->add($this->pagina['titulo']);
        ...
        $this->corpo = $this->load->view('noticia', $v, true);
